...
def Corr_Heatmap_VII_40under(_project_name,_df_path,sample_list_path,out_path, corr_method='pearson'):
    # Correlation Heatmap edits

    # read the normalized counts in
    #df_path = _df_path
    df_unordered = pd.read_csv(str(_df_path), sep='\t', header='infer', index_col=0)
    # reset indices because this can cause an issue
    df_unordered = df_unordered.reset_index()
    ...
    ordered_sample_names = sample_list[0].tolist()
    ...
    ax.set_xticklabels(_cols_cpm, ha='right', rotation=25, fontsize=105) # 10 larger x axis ticks and labels remove


    ax.set_yticks(tick_pos)
    ax.set_yticklabels(_cols_cpm, ha='right', va='top',rotation=25, minor=False, fontsize=105, y=tick_pos)
    ...
    ax.collections[0].colorbar.set_label(f"{corr_method.capitalize()} Correlation Coefficient",fontsize = 14) 
    ax.collections[0].colorbar.ax.tick_params(labelsize=12)

    #fig.savefig(str(out_path)+'/'+name_underscores+'_Pearson_Correlation_Heatmap.png', dpi=300) # inlcude '/' before and after the out_path argument
    fig.savefig(f"{out_path}/{name_underscores}_{corr_method.capitalize()}_Correlation_Heatmap.png", dpi=300)
    plt.close()
    return None
