...
#is_pairwise <- if (length(args) >= 5) as.character(args[5]) else NA
# Read counts data -----------------------
...
print("counts_data_colfilter")
head(counts_data_colfilter)
...
# 3/29/2023
#counts_data_colfilter <- counts_data_colfilter %>% mutate(across(everything(),as.numeric))
...
# FOR loop for putting condition2 as the second item in the contrasts of interest list - which is the REFERENCE condition
