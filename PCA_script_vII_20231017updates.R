...
color_mapping <- setNames(conds_file$V3, conds)
...
# Create a combined factor for legend
pca_df$combined_factor <- interaction(pca_df[,3], pca_df$shape_mapping)
#pca_df$combined_factor <- factor(paste(pca_df$conds, pca_df$shape_mapping, sep="_"))

# Map the colors and shapes to the levels of the new factor
color_levels <- unique(pca_df$combined_factor)
shape_levels <- as.numeric(pca_df$shape_mapping)
print(color_levels)
print(shape_levels)
#color_mapping <- setNames(as.character(pca_df[,3]), color_levels)
#shape_mapping <- setNames(shape_levels, color_levels)

# Create named vector for original color mapping
#color_mapping_original <- setNames(conds_file$V3, conds_file$V1)

# Map the combined factor levels to the original colors
color_mapping <- setNames(color_mapping[as.character(pca_df[,3])], color_levels)

## PCA plot without labels
jpeg(paste0(file_prefix[[1]][1],"_PCAplot.jpg"), height= 10, width= 12, units = "in", res = 600)

pca_df %>%
  ggplot(aes(x=PC1,y=PC2, label=sampleLab, color= combined_factor, shape = combined_factor)) +
  geom_point(size=3) +
  scale_shape_manual(values=shape_mapping) +
  scale_colour_manual(values= color_mapping) + # 12/02/2022 - TylerT manually putting in the sample numbers b/c code not work
  labs(x=paste0("PC1: ",round(percentVar[1]*100,1), "% variance"),
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
        legend.title = element_text(size=12),legend.text = element_text(size = 10),legend.position = "right") + # Adjust this value to change the label size in the legend
...
jpeg(paste0(file_prefix[[1]][1],"_PCAplot_labelled.jpg"), height=10, width=12, units="in", res=600)
...
pca_df %>%
  ...
  labs(x=paste0("PC1: ",round(percentVar[1]*100,1), "% variance"),
       y=paste0("PC2: ",round(percentVar[2]*100,1), "% variance")) +
  geom_label_repel(size=3.2, color="blue", box.padding= unit(0.5, 'lines'), point.padding= unit(0.6, 'lines'), # 5/3/2023 - loosening up the repel criteria
  arrow = arrow(length = unit(0.0099, 'npc')), segment.color='blue', segment.size=0.2) +
  theme_linedraw(base_size=16) + # 1/17/2023 - increase the PCA font size for generated figure
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        legend.title = element_text(size=12),legend.text = element_text(size = 10),legend.position = "right") + # Adjust this value to change the label size in the legend
  guides(color = guide_legend(ncol = 1))
