...
FPKM_HQsamples_GoI_ordered <- FPKM_HQsamples_GoI %>% select(all_of(new_order))
...
hclust_gene_normalized_FPKM_HQsamples_GoI <- hclust(dist(normalized_FPKM_HQsamples_GoI), method = "complete")
as.dendrogram(hclust_gene_normalized_FPKM_HQsamples_GoI) %>%
  plot(horiz = TRUE)


hclust_FPKM_HQsamples_GoI_gene_col <- cutree(tree = as.dendrogram(hclust_gene_normalized_FPKM_HQsamples_GoI), k = 2)
my_gene_col <- data.frame(Cluster = ifelse(test = hclust_FPKM_HQsamples_GoI_gene_col == 1, yes = "Gene Cluster 1", no = "Gene Cluster 2"))
head(my_gene_col)
write.csv(my_gene_col, 
          file = paste0(output_directory, "Hierarchical_Sample_Gene_Clusters_",model_name, ".csv"), 
          row.names = TRUE)
...
annots_HM_FPKM_HQsamples_GoI <- pheatmap(normalized_FPKM_HQsamples_GoI, annotation_row = my_gene_col, cluster_cols = FALSE, cutree_rows = 2, fontsize_row = .5, main = paste0("Gene Expression Heatmap: ",model_name), treeheight_row = 30, angle_col = 315, fontsize_col = 7, width = 10, height = 12, gaps_col = gaps_indices)
ggsave(filename = paste0(output_directory,"Hierarchical_Sample_Gene_Expression_Heatmap_",model_name,".pdf"),plot = annots_HM_FPKM_HQsamples_GoI, width = 10, height = 12)



# HEATMAP 3 - sample group averages, column order same as specified, samples are clustered ++++++++++++++++++++++++++++++++++++++++++++++++++
...
hclust_gene_merged_exp_grps_df <- hclust(dist(merged_exp_grps_df), method = "complete")
as.dendrogram(hclust_gene_merged_exp_grps_df) %>%
  plot(horiz = TRUE)


hclust_merged_exp_grps_genes_col <- cutree(tree = as.dendrogram(hclust_gene_merged_exp_grps_df), k = 2)
merged_exp_grps_gene_col <- data.frame(Cluster = ifelse(test = hclust_merged_exp_grps_genes_col == 1, yes = "Gene Cluster 1", no = "Gene Cluster 2"))
head(merged_exp_grps_gene_col)
write.csv(merged_exp_grps_gene_col, 
          file = paste0(output_directory,"Hierarchical_ExpGrps_Gene_Clusters_",model_name, ".csv"), 
          row.names = TRUE)
...
ExpGrp_HeatMap <- pheatmap(merged_exp_grps_df, annotation_row = merged_exp_grps_gene_col, cluster_cols = FALSE, cutree_rows = 2, fontsize_row = .5, main = paste0("Experimental Group Gene Expression Heatmap: ",model_name), treeheight_row = 30, angle_col = 315, fontsize_col = 7, width = 10, height = 12, gaps_col = gaps_indices2)
ggsave(filename = paste0(output_directory,"Hierarchical_Experimental_Group_Gene_Expression_Heatmap_",model_name,".pdf"),plot = ExpGrp_HeatMap, width = 8, height = 12)
