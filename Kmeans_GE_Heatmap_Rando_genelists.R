...
# FXN to filter GoI list based on existing Gene Symbol Names from the bulk pipeline counts file
# filter the FPKM counts based on the genes of interest before selecting the HQ samples
# Get the indices of the matching rows
indices <- unique(which(FPKM$Gene_Symbol %in% GoI_Cuda$GoI))
# Subset the data frame using the indices
FPKM_GoI <- unique(FPKM[indices, ])
...
rownames(normalized_FPKM_HQsamples_GoI) <- FPKM_GoI$Gene_Symbol # assign row names back to gene counts file

# Omit any NaN values - 5/17/2023 b/c certain gene lists may not contain the genes listed in the bulk pipeline output if it is a GoI list found from the literature
normalized_FPKM_HQsamples_GoI <- normalized_FPKM_HQsamples_GoI[!apply(is.nan(normalized_FPKM_HQsamples_GoI), 1, any), ]
normalized_FPKM_HQsamples_GoI <- unique(normalized_FPKM_HQsamples_GoI)
normalized_FPKM_HQsamples_GoI_rownames <- as.list(rownames(unique(normalized_FPKM_HQsamples_GoI)))
normalized_FPKM_HQsamples_GoI_rownames <- make.names(normalized_FPKM_HQsamples_GoI_rownames, unique = TRUE) 
# for any duplicated names that are not removed b/c the gene names are the same but the ensemble IDS are different - prevents any data loss while maintaining gene ID
...
rownames(my_gene_col_2clusters) <- normalized_FPKM_HQsamples_GoI_rownames
head(my_gene_col_2clusters)
...
rownames(my_gene_col_4clusters) <- normalized_FPKM_HQsamples_GoI_rownames
head(my_gene_col_4clusters)
...
annots_HM_FPKM_HQsamples_2clusters <- pheatmap(normalized_FPKM_HQsamples_GoI, annotation_row = my_gene_col_2clusters, cluster_cols = FALSE, cutree_rows = 2, fontsize_row = .5, main = paste0("K-means Gene Expression Matrix (2 clusters) - ",model_name), treeheight_row = 30, angle_col = 315, fontsize_col = 7, width = 10, height = 12, gaps_col = gaps_indices)
...
avg_exp_groups <- lapply(subset_list, function(x) as.data.frame(rowMeans(x)))

labeled_genes_avg_exp_grps <- lapply(seq_along(avg_exp_groups), function(x) {
  df <- avg_exp_groups[[x]]
  rownames(df) <- normalized_FPKM_HQsamples_GoI_rownames
  return(df)
})
